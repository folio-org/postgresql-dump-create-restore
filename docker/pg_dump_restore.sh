...
if [ "$ACTION" == backup ]; then
  (
    ...
    aws s3 cp $EBS_VOLUME_MOUNT_PATH/$DB_BACKUP_NAME.psql $S3_BACKUPS_BUCKET/$RANCHER_CLUSTER_NAME/$RANCHER_PROJECT_NAME/$DB_BACKUP_NAME/
    echo "AWS s3 cp operation of file $EBS_VOLUME_MOUNT_PATH/$DB_BACKUP_NAME.psql to s3 $S3_BACKUPS_BUCKET/$RANCHER_CLUSTER_NAME/$RANCHER_PROJECT_NAME/$DB_BACKUP_NAME/ bucket SUCCESSFULLY COMPLETED"
    ...
    aws s3 cp $EBS_VOLUME_MOUNT_PATH/"$DB_BACKUP_NAME"_install.json $S3_BACKUPS_BUCKET/$RANCHER_CLUSTER_NAME/$RANCHER_PROJECT_NAME/$DB_BACKUP_NAME/
    echo "AWS s3 cp operation of file $EBS_VOLUME_MOUNT_PATH/"$DB_BACKUP_NAME"_install.json to s3 $S3_BACKUPS_BUCKET/$RANCHER_CLUSTER_NAME/$RANCHER_PROJECT_NAME/$DB_BACKUP_NAME/ bucket SUCCESSFULLY COMPLETED"
    ...
    aws s3 cp $EBS_VOLUME_MOUNT_PATH/"$DB_BACKUP_NAME"_okapi_install.json $S3_BACKUPS_BUCKET/$RANCHER_CLUSTER_NAME/$RANCHER_PROJECT_NAME/$DB_BACKUP_NAME/
