...
if [ "$ACTION" == backup ]; then
  ...
  (
    set -e
    aws s3 cp $AWS_BUCKET/$RANCHER_CLUSTER_PROJECT_NAME/$JENKINS_DB_BACKUP_NAME $EBS_VOLUME_MOUNT_PATH
    echo "AWS s3 cp operation of file $EBS_VOLUME_MOUNT_PATH/$JENKINS_DB_BACKUP_NAME to $EBS_VOLUME_MOUNT_PATH path SUCCESSFULLY COMPLETED"
