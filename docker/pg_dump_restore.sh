...
if [ "$ACTION" == backup ]; then
  ...
  (
    set -e
    aws s3 cp "s3://$JENKINS_DB_BACKUP_NAME.psql" $EBS_VOLUME_MOUNT_PATH
    echo "AWS s3 cp operation of file $JENKINS_DB_BACKUP_NAME.psql to $EBS_VOLUME_MOUNT_PATH path SUCCESSFULLY COMPLETED"
    PGPASSWORD=$DB_PASSWORD psql -h $DB_HOST -U $DB_USERNAME -p $DB_PORT < "$EBS_VOLUME_MOUNT_PATH/$JENKINS_DB_BACKUP_NAME.psql" > /dev/null
    echo "psql restore operation SUCCESSFULLY COMPLETED. Path to file is $EBS_VOLUME_MOUNT_PATH/$JENKINS_DB_BACKUP_NAME.psql"
    )
    errorCode=$?
    if [ $errorCode -ne 0 ]; then
